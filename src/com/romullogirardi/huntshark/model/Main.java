...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		//Computing best results to indirect combinations considering all past contests
		System.out.println("Computando combinações indiretas considerando todos os concursos passados...");
		bestGameStrategies = new ArrayList<>();
		ContestManager.newInstance();
		ContestManager.getInstance().setState(State.INDIRECT_ALL);
		ContestManager.getInstance().populateContests();
		System.out.println("RANKING DAS COMBINAÇÕES INDIRETAS CONSIDERANDO TODOS OS CONCURSOS PASSADOS:");
		for(GameStrategy gameStrategy : bestGameStrategies) 
			System.out.println(gameStrategy.toString());
		System.out.println();

		//Computing best results to direct combinations considering all past contests
		System.out.println("Computando combinações diretas considerando todos os concursos passados...");
		bestGameStrategies = new ArrayList<>();
		ContestManager.newInstance();
		ContestManager.getInstance().setState(State.DIRECT_ALL);
		ContestManager.getInstance().populateContests();
